...
public class RemoteWorker implements RemoteWorkGrpc.RemoteWork {
  ...
  public RemoteWorker(Path workPath, RemoteOptions remoteOptions,
                      RemoteWorkerOptions options, ConcurrentMap<String, byte[]> cache) {
  ...
  @Override
  public void executeSynchronously(RemoteWorkRequest request,
                                   StreamObserver<RemoteWorkResponse> responseObserver) {
    ...
    try {
      ...
      final MemcacheWorkExecutor workExecutor = new MemcacheWorkExecutor(
          actionCache, tempRoot);
      if (options.debug) {
        System.out.println("[INFO] Work received has "+ request.getInputFilesCount() +
                           " inputs files and " + request.getOutputFilesCount() + " output files.");
      }
  ...
  public static void main(String[] args) throws Exception {
    ...
    Path workPath = getFileSystem().getPath(remoteWorkerOptions.workPath);
      ...
      RemoteWorker worker = new RemoteWorker(
          ...
          remoteWorkerOptions,
          cache);
    Server server = ServerBuilder.forPort(remoteWorkerOptions.listenPort).addService(
        RemoteWorkGrpc.bindService(worker)).build();
    server.start();
