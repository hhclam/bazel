...
public class HazelcastCacheFactory {
  ...
  public Cache<String, byte[]> create(RemoteOptions options) {
    ...
    if (options.hazelcastConfiguration != null) {
      ...
      prop.setProperty("hazelcast.config.location", options.hazelcastConfiguration);
    }
    if (options.hazelcastNode != null) {
      ...
      String instanceName = HazelcastClient.newHazelcastClient(config).getName();
      prop.setProperty("hazelcast.instance.name", instanceName);
    }
